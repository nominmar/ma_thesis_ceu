from CTL2.causal_tree.ctl.honest import *
from CTL2.causal_tree.ctl.ctl_base import *
from CTL2._tree import _CausalTree
...
class CausalTree(_CausalTree):
    ...
    def __init__(self, cont=False, val_honest=False, honest=False, min_size=2, max_depth=-1, split_size=0.5, weight=0.5,
                 ...
                 max_values=None, verbose=False, est_size = 0.5):
        super().__init__()
        self.cont = cont
        ...
        if cont:
            ...
            elif honest and (split_size <= 0.0 or weight <= 0.0): #AtheyImbens
    ...
    def fit(self, x, y, t, est_size):
        self.column_num = x.shape[1]
        x = x.astype(np.float)
        y = y.astype(np.float)
        t = t.astype(np.float)
        self.tree.fit(x, y, t, est_size)
        self.fitted = True
